...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<xsl:param name="welcomeMsg">
		...
		<div>This archive includes a few examples of editions created using EVT,
			you will now be shown a small excerpt of the
			<a href="http://pelavicino.labcd.unipi.it/" target="blank">
				Codice Pelavicino Digitale</a> edition.
		</div>
		<div>It is recommended to go to full screen mode so that all available screen space is used to show the manuscript
			images and the transcription text.</div>
		<div>For more information refer to the EVT Manual in the "doc" folder. If you have any suggestions or spot an
			error/bug please contact us at <a href="mailto:evt.developers@gmail.com">evt.developers@gmail.com</a></div>
		<br/>
		<div>Please wait a few seconds before closing this window, EVT is processing the document list.</div>
	</xsl:param>
	...
	<xsl:param name="viscoll_scheme_path">text/viscoll/CP-quireStructure.xml</xsl:param>
